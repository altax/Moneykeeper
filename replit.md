...
## Последние изменения
- 2025-12-05: UI/UX REDESIGN v2 - Просторный и современный интерфейс:
  - Переработана секция завершённых смен: дата (день + месяц), сумма заработка над столбиком
  - Высота столбиков пропорциональна заработку для визуального сравнения
  - Увеличены отступы на главном экране для менее компактного вида
  - Улучшены карточки активных и запланированных смен (больше spacing)
  - Исправлен баг splash screen в Expo Go (иконка теперь скрывается корректно)
  - Убрано ограничение на количество смен - можно записываться неограниченно
  - Вместо автозавершения смен через 12 часов теперь показывается модальное окно
  - Модальное окно завершения смены: обязательный ввод заработка, выбор цели, предупреждение при отправке в сейф
...
### Реализовано:
- Профессиональный тёмный дизайн в стиле банковского приложения
- Просторный и современный UI с увеличенными отступами
...
- Неограниченное количество смен на любую дату
- Ручное завершение смен с модальным окном (ввод заработка, выбор цели, предупреждение)
- Отображение времени до начала запланированных смен
- Секция активных смен с прогресс-баром и кнопкой "Завершить"
- Секция завершённых смен: график с датами, суммой над столбиками и пропорциональной высотой
